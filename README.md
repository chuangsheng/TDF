...
TDF used scene example:  
  ...
  TDF is a C/S program, based python version 3. next, shows you TDF work flow:  
    ...
    7. Server program every time check plugins status when get client program result, if __current plugins__status is finish, send signal to plugins   
    8. Server plugins get finish signal, to do something to closure.  
    
    __Welcome to perfect TDF with me!__(My english and program is bad:-D)
